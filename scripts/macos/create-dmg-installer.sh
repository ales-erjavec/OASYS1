...
cp -a "${APP}" "${TMP_TEMPLATE}"/Oasys1.2.app

if [[ "${IDENTITY}" ]]; then
    codesign -s "${IDENTITY}" --deep --verbose \
        "${TMP_TEMPLATE}"/Oasys1.2.app
fi
...
hdiutil create -format UDRW -volname Oasys1.2 -fs HFS+ \
